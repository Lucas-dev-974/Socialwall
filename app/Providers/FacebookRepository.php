...
class FacebookRepository
{
    ...
    public function getLongLiveToken($accessToken){
        try{
            $response = $this->facebook->get('/oauth/access_token?grant_type=fb_exchange_token&client_id=' . config('providers.facebook.app_id') . '&client_secret=' . config('providers.facebook.app_secret') . '&fb_exchange_token=' . $accessToken . '&accessToken=' . $accessToken);
            return $response;
        }catch(\Facebook\Exceptions\FacebookResponseException $e) {
            // return var_dump($e);
