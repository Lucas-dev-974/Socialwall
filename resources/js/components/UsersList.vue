<template>
    <div  id="dash-left">
        <div class="py-2 w-100 bg- d-flex justify- ">
            <div class="w-90">
                <v-text-field name="name" id="id" label="Chercher un utilisateur" v-model="search" dense rounded filled></v-text-field>
            </div>
        </div>

        <v-simple-table dark fixed-header class="mx-auto">
            <template>
                <thead>
                    <tr>
                        <th>Utilisateur</th> <th>Email</th> <th>Mot de passe</th> <th>Phone</th> <th class="text-center">Wall</th>
                        <th>Action</th>
                    </tr>
                </thead>
                ...
                <tbody >
                    <tr v-for="user in $store.state.users" :key="user.id" class="user-table-tr">
                        ...
                        <td>{{user.email}}</td> 
                        <td><a href="#" class="custom-link">Envoyer un mail</a> </td>
                        <td>{{user.phone}}</td>

                        <td>
                            <div class="d-flex align-center my-0 py-0">
                                <v-menu offset-y  min-width="300px" class="my-0 py-0" :close-on-content-click="false">
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-btn color="rgb(5 184 255)" dark v-bind="attrs" v-on="on" x-small outlined class="my-0 mx-auto">
                                            Walls
                                        </v-btn>
                                    </template>
